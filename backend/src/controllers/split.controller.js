...
export const searchwbticket = async (req, res) => {
...
export const amendSplittingToSingleSO = async (req, res) => {
...
export const getWbTicketsByMWerksAndDate = async (req, res) => {
  const { mwerks, WBDATE_IN } = req.query;
  console.log('getWbTicketsByMWerksAndDate params:', mwerks, WBDATE_IN);
  if (!mwerks || !WBDATE_IN) {
  ...
  try {
    ...
    const result = await pool
      ...
      .query(`
        ...
        ORDER BY WB_TICKET
      `);
    console.log('Tickets found:', result.recordset.length);
